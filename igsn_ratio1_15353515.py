...
for EC in [5]:#range(len(energy_a)):
    ...
    ######################现在在fB的基础上，要减去各个agent消耗的能量######################
        ...
        np.save('ratio1_EC5_all_type_results.npy',all_type_results)
        np.save('ratio1_EC5_tomagent_sv_ratio.npy',tomagent_sv_ratio)
        np.save('ratio1_EC5_tommat.npy',tommat)
        np.save('ratio1_EC5_fB.npy',fB)
        np.save('ratio1_EC5_whichEC',saveEC)
        np.save('ratio1_EC5_whichstep',checkstep)
