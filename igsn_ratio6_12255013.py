...
for EC in range(len(energy_a)):
    ...
    ######################现在在fB的基础上，要减去各个agent消耗的能量######################
        ...
        np.save('ratio6_all_type_results.npy',all_type_results)
        np.save('ratio6_tomagent_sv_ratio.npy',tomagent_sv_ratio)
        np.save('ratio6_tommat.npy',tommat)
        np.save('ratio6_fB.npy',fB)
        np.save('ratio6_whichEC',saveEC)
        np.save('ratio6_whichstep',checkstep)
